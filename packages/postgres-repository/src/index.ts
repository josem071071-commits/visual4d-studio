...
const artifactTables: Record<ArtifactKind, string> = {
...
function artifactStage(kind: ArtifactKind): string {
  return ({
    ...
    DESIGN: "GENERATED"
  } satisfies Record<ArtifactKind,string>)[kind];
}

function rowInstitution(r: any): InstitutionRecord {
...
function rowAsset(r:any): AssetRecord {
...
function rowAssetVersion(r:any): AssetVersionRecord {
  return { id:r.id, assetId:r.asset_id, status:r.status, checksumSha256:r.checksum_sha256 };
}
function rowArtifact(r:any, kind:ArtifactKind): ArtifactVersionRecord {
  return { id:r.id, projectId:r.project_id, kind, versionNumber:r.version_number, payload:r.payload };
}

export interface PostgresRepositoryOptions { connectionString?: string; pool?: PgPool; }

export class PostgresProjectRepository implements ProjectRepository {
  readonly pool: PgPool;
  private readonly ownsPool: boolean;
  private readonly txStorage = new AsyncLocalStorage<any>();

  private db(): Queryable { return this.txStorage.getStore() ?? this.pool; }
  private async inTransaction<T>(action:(db:Queryable)=>Promise<T>):Promise<T>{
  ...
  async getInstitution(id:string) {
    const q=await this.db().query("SELECT * FROM institutions WHERE id=$1",[id]); return q.rows[0] ? rowInstitution(q.rows[0]) : null;
  }
  async saveInstitution(r:InstitutionRecord) {
    await this.db().query(`INSERT INTO institutions(id,owner_user_id,name,status,active_identity_version_id)
      VALUES($1,$2,$3,$4,$5)
      ON CONFLICT(id) DO UPDATE SET name=EXCLUDED.name,status=EXCLUDED.status,active_identity_version_id=EXCLUDED.active_identity_version_id,updated_at=now()`,
      [r.id,r.ownerUserId,r.name,r.status,r.activeIdentityVersionId]);
  }
  async getIdentityVersion(id:string) {
    const q=await this.db().query("SELECT * FROM identity_versions WHERE id=$1",[id]); return q.rows[0] ? rowIdentity(q.rows[0]) : null;
  }
  async listIdentityVersions(institutionId:string) {
    const q=await this.db().query("SELECT * FROM identity_versions WHERE institution_id=$1 ORDER BY version_number",[institutionId]); return q.rows.map(rowIdentity);
  }
  async saveIdentityVersion(r:IdentityVersionRecord) {
  ...
  async activateIdentityVersion(institutionId:string, identityVersionId:string) {
    return this.inTransaction(async c=>{
      ...
      if (!inst.rowCount) throw new Error("INSTITUTION_NOT_FOUND");
      ...
      if (!target.rowCount) throw new Error("IDENTITY_ACTIVATION_MISMATCH");
  ...
  async getProject(id:string) { const q=await this.db().query("SELECT * FROM projects WHERE id=$1",[id]); return q.rows[0] ? rowProject(q.rows[0]) : null; }
  async saveProject(p:ProjectRecord, expectedRevision?:number) {
    if (expectedRevision === undefined) {
      await this.db().query(`INSERT INTO projects(id,owner_user_id,institution_id,identity_version_id,project_type,title,format_width,format_height,orientation,current_stage,status,revision,final_design_version_id)
        ...
        ON CONFLICT(id) DO UPDATE SET current_stage=EXCLUDED.current_stage,status=EXCLUDED.status,final_design_version_id=EXCLUDED.final_design_version_id,updated_at=now()`,
        [p.id,p.ownerUserId,p.institutionId,p.identityVersionId,p.projectType,p.title,p.width,p.height,p.orientation,p.currentStage,p.status,p.revision,p.finalDesignVersionId]);
      return;
    }
    ...
    if (q.rowCount !== 1) throw new Error("PROJECT_REVISION_CONFLICT");
  }
  async listAssets(institutionId:string) { const q=await this.db().query("SELECT * FROM assets WHERE institution_id=$1 ORDER BY id",[institutionId]); return q.rows.map(rowAsset); }
  async getAssetVersion(id:string) { const q=await this.db().query("SELECT * FROM asset_versions WHERE id=$1",[id]); return q.rows[0] ? rowAssetVersion(q.rows[0]) : null; }

  private async artifactFromTable(db:Queryable, id:string, kind:ArtifactKind) {
  ...
  async createArtifactVersion(input:Omit<ArtifactVersionRecord,"id"|"versionNumber">) {
    return this.inTransaction(async c=>{
      ...
      const versionNumber=Number(v.rows[0].next); const id=`${input.kind.toLowerCase()}_${input.projectId}_v${versionNumber}`;
      if (input.kind === "VERIFICATION") {
        const designVersionId=(input.payload as any)?.designVersionId ?? null;
  ...
  async getArtifactVersion(id:string) {
    for (const kind of Object.keys(artifactTables) as ArtifactKind[]) { const a=await this.artifactFromTable(this.db(),id,kind); if(a) return a; } return null;
  }
  async getLatestArtifact(projectId:string,kind:ArtifactKind) {
    const table=artifactTables[kind]; const q=await this.db().query(`SELECT id,project_id,version_number,payload FROM ${table} WHERE project_id=$1 ORDER BY version_number DESC LIMIT 1`,[projectId]);
    return q.rows[0] ? rowArtifact(q.rows[0],kind) : null;
  }
  async saveApproval(a:ApprovalRecord) {
    await this.db().query(`INSERT INTO approvals(project_id,stage,artifact_type,artifact_version_id,decision,origin,approved_by_user_id,approved_at,decided_at)
      ...
      ON CONFLICT(project_id,artifact_type,artifact_version_id,decision) DO UPDATE SET origin=EXCLUDED.origin,approved_by_user_id=EXCLUDED.approved_by_user_id,approved_at=EXCLUDED.approved_at,decided_at=now()`,
      [a.projectId,artifactStage(a.artifactKind),a.artifactKind,a.artifactVersionId,a.decision,a.origin,a.approvedByUserId,a.approvedAt]);
  }
  ...
  async runIdempotentMutation<T>(input:IdempotentMutationInput, action:()=>Promise<T>):Promise<T>{
    return this.inTransaction(async c=>{
      ...
      if(reserve.rowCount===0){
        ...
        if(row?.status==='COMPLETED') return row.result_json as T;
        if(row?.status==='IN_PROGRESS') throw new Error('IDEMPOTENCY_REQUEST_IN_PROGRESS');
