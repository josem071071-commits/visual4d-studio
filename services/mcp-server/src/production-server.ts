...
export function buildProductionMcpServer(workflow:ProjectWorkflowService,grants:PostgresApprovalGrantStore):McpServer{
 ...
 const server=new McpServer({name:"visual4d-production",version:"0.4.6"},{capabilities:{tools:{},resources:{}}});
...
export function createProductionMcpHttpServer(options:ProductionMcpServerOptions){
 ...
 const server=createServer(async(req,res)=>{
  if(req.url==="/healthz")return json(res,200,{ok:true,service:"visual4d-mcp-production",version:"0.4.6"});
...
if(process.argv[1]&&new URL(import.meta.url).pathname===process.argv[1]){
 const databaseUrl=process.env.DATABASE_URL,issuer=process.env.VISUAL4D_OIDC_ISSUER,audience=process.env.VISUAL4D_OIDC_AUDIENCE,jwksUri=process.env.VISUAL4D_OIDC_JWKS_URI;
 if(!databaseUrl||!issuer||!audience||!jwksUri)throw new Error("DATABASE_URL, VISUAL4D_OIDC_ISSUER, VISUAL4D_OIDC_AUDIENCE and VISUAL4D_OIDC_JWKS_URI are required");
 const verifier=new Rs256JwksTokenVerifier({issuer,audience,jwksUri});const app=createProductionMcpHttpServer({databaseUrl,verifier,host:process.env.VISUAL4D_MCP_HOST??"0.0.0.0",port:Number(process.env.PORT??process.env.VISUAL4D_MCP_PORT??8787),rateLimitPerMinute:Number(process.env.VISUAL4D_RATE_LIMIT_PER_MINUTE??120)});const{baseUrl}=await app.listen();console.error(`Visual 4D production OAuth MCP listening at ${baseUrl}/mcp`);
}
