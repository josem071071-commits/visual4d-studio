...
export function buildProductionMcpServer(workflow:ProjectWorkflowService,grants:PostgresApprovalGrantStore):McpServer{
 ...
 const server=new McpServer({name:"visual4d-production",version:"0.4.10"},{capabilities:{tools:{},resources:{}}});registerRenderPreviewResource(server);
...
export function createProductionMcpHttpServer(options:ProductionMcpServerOptions){
 ...
 const server=createServer(async(req,res)=>{
  ...
  if(requestUrl.pathname==="/healthz")return json(res,200,{ok:true,service:"visual4d-mcp-production",version:"0.4.10"});
